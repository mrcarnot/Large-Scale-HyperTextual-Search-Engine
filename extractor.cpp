...
static string get_string_member(const Value &obj, const char *key) {
    ...
    // if not string, try to stringify fallback
    if (v.IsNumber()) {
...
static string join_authors(const Document &doc) {
    // Expect metadata.authors array of objects with first/middle/last or name
    ...
    for (const auto &a : meta["authors"].GetArray()) {
        if (a.IsObject()) {
            ...
            // middle might be array or string
            string middle;
...
static string extract_abstract(const Document &doc) {
    // 1) top-level "abstract" (array of objects or strings)
    ...
    // 2) search body_text sections for "Abstract"
...
static string extract_doi(const Document &doc) {
    // try metadata.doi, metadata.other_ids, bib_entries -> other_ids -> DOI
...
int main(int argc, char** argv) {
    ...
    for (const auto &path : files) {
        ...
        if (sections.empty()) {
            // fallback: join all body_text texts
            if (doc.HasMember("body_text") && doc["body_text"].IsArray()) {
                string f;
                for (const auto &bt : doc["body_text"].GetArray()) {
        ...
        // optional JSONL summary using RapidJSON writer
        if (jofs.is_open()) {
